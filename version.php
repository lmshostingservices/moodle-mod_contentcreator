...
$plugin->release   = '13.65'; // FIX-CC-ROUTE-CARDCOUNT (v13.65): validateCards() computed the expected card count as (mode === 'vet') ? 7 : 6, but prompts.js instructs the model to return exactly 7 cards for vet, workplace AND pd (only university asks for 6). Workplace and PD sections therefore returned exactly what they were told to and were rejected with "Expected 6 cards, got 7" on attempt 1 AND the attempt-2 repair pass, so generateFiveCardSequence() always fell through to getFailedCardSequence() - a 100% failure rate on those two routes, rendering "AI generation failed ... use Regenerate Failed to retry" placeholder cards while burning full generation credits. validateCards() now defers to the canonical Prompts.getCardCountForMode() so validator and prompt can never drift apart again. Also: FIX-CC-UNIVERSITY-CARDTYPES (getExpectedCardOrder() ignored its mode argument and always returned the 7 VET types, so normalizeCardSchema() force-rewrote concept-anchor->hook-scenario and analytical-lens->mental-model, failing validation with "mental-model: requires at least 3 steps"); FIX-CC-VO-STRUCTURAL (validateCards() demanded 30+ char voiceoverText on cards where normaliseAllVoiceovers() deliberately leaves it empty because player5.js builds narration from structural fields); FIX-CC-ACTIVITIES-CONTEXT (activitiesEnabled now surfaced on context so the card-count check sees it); FIX-CC-POLL-CEILING (MAX_POLLS 50->120, ~150s->~6min: ajax.php already allowed the server 180s so the browser was abandoning jobs still in progress); FIX-CC-POLL-TOLERANCE (a single transient poll failure abandoned a live job and started a new BILLABLE one; now absorbs 5 consecutive failures); FIX-CC-POLL-AUTH (poll_job had no require_sesskey, no login and no capability check - now enforced with cmid + contentcreator_require_manage + write_close; pollJob() sends &cmid= so AMD build and ajax.php MUST deploy together). AMD: generator.js, cc-state.js (src+build+min, map regenerated). PHP: ajax.php, db/upgrade.php, version.php. No CSS or DB schema changes. Retains the v13.64 10-digit numbering scheme; version.php -> 2026081700, savepoint 2026081700. // FIX-CC-13DIGIT-SAVEPOINT-REBASE (v13.64): db/upgrade.php rebased all 193 legacy 13-digit upgrade gates/savepoints to 10-digit (max 2026072801, below $plugin->version). The previous file's 13-digit savepoints (max 2026072800232) re-wrote a 13-digit version into config_plugins during upgrade on rebased 10-digit sites, silently re-stranding them (core's final version write is skipped because installedversion > $plugin->version, and every later upgrade run then aborts with downgrade_exception). No schema, PHP logic, JS or lang changes — upgrade.php numbering only, plus pipeline-ignore marketplace-checker annotations on pre-existing raw-parameter lines (comments only).
